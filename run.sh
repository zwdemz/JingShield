...
source_checkout=0
source_binary=""
if [[ -f "${script_dir}/payload/jingshield" ]]; then
	package_mode=1
elif [[ -f "${script_dir}/configs/config.yaml" && -f "${script_dir}/deploy/linux/jingshield.service" ]]; then
	# Allow the repository checkout to be installed directly. This is useful for
	# an offline lab node where a prebuilt binary is kept at the repository root
	# or in bin/; the same provisioning path is used as for a release package.
	for candidate in "${script_dir}/bin/jingshield-linux-amd64"; do
		if [[ -f "${candidate}" ]]; then
...
if [[ "${source_checkout}" -eq 1 ]]; then
  config_file="${JINGSHIELD_CONFIG:-${script_dir}/configs/config.yaml}"
else
  config_file="${JINGSHIELD_CONFIG:-${install_root}/config.yaml}"
fi
env_file="${JINGSHIELD_ENV_FILE:-${install_root}/jingshield.env}"
if [[ "${source_checkout}" -eq 1 ]]; then
  service_file="${JINGSHIELD_SERVICE_FILE:-${script_dir}/jingshield.service}"
else
...
Usage:
  ...
  Install directly from a repository checkout: sudo ./run.sh [options]
  Start an installed node:             sudo /opt/jingshield/run.sh

Options for a complete package or repository checkout:
...
required_commands=(install sha256sum od tr mysql readlink sed)
if [[ "${source_checkout}" -eq 0 ]]; then
  required_commands+=(systemctl getent useradd groupadd)
fi
...
env_file="${JINGSHIELD_ENV_FILE:-${install_root}/jingshield.env}"
if [[ "${source_checkout}" -eq 1 ]]; then
	required_files=("${source_binary}" "${script_dir}/configs/config.yaml" "${script_dir}/deploy/linux/jingshield.service")
else
	required_files=("${script_dir}/payload/jingshield" "${script_dir}/config/config.yaml" "${script_dir}/systemd/jingshield.service" "${script_dir}/upgrade.sh")
fi
...
if [[ "${source_checkout}" -eq 0 ]]; then
	...
	echo "detected JingShield repository checkout; using prebuilt binary ${source_binary}"
fi
...
explicit_db_password="${JINGSHIELD_DB_PASS:-}"
if [[ -f "${env_file}" ]]; then
  load_values "${env_file}" 0
elif [[ "${env_file}" != "${legacy_env_file}" && -f "${legacy_env_file}" ]]; then
  if [[ "${source_checkout}" -eq 1 ]]; then
    echo "importing credentials from legacy layout for repository mode: ${legacy_env_file}"
  else
...
# Repository/package deployments keep the database password in the active YAML
# configuration. Environment files remain supported as a compatibility layer.
if [[ -z "${explicit_db_password}" && -f "${config_file}" ]]; then
  config_db_password="$(awk '
    /^[[:space:]]*database:[[:space:]]*$/ {in_db=1; next}
    in_db && /^[^[:space:]]/ {in_db=0}
    in_db && /^[[:space:]]*pass:[[:space:]]*/ {
      value=$0
      sub(/^[^:]*:[[:space:]]*/, "", value)
      sub(/[[:space:]]+#.*$/, "", value)
      gsub(/^\"|\"$/, "", value)
  ...
  ' "${config_file}")"
  if [[ -n "${config_db_password}" ]]; then
    export JINGSHIELD_DB_PASS="${config_db_password}"
  fi
fi
...
prompt_db_password() {
  if [[ -z "${db_password}" && -t 0 ]]; then
    read -r -s -p "MySQL 密码（${db_user}@${db_host}）: " db_password
    echo
  fi
}
...
persist_db_password_to_config() {
  local config_tmp
  config_tmp="${config_file}.tmp.$$"
  sed -E "
    /^[[:space:]]*database:[[:space:]]*$/,/^[^[:space:]]/ {
      s|^([[:space:]]*pass:)[[:space:]]*.*$|\\1 \"${db_password}\"|
    }
  " "${config_file}" > "${config_tmp}"
  if ! cmp -s "${config_file}" "${config_tmp}"; then
    chmod --reference="${config_file}" "${config_tmp}" 2>/dev/null || chmod 0640 "${config_tmp}"
...
if [[ "${source_checkout}" -eq 0 ]]; then
  ...
  install -d -m 0750 "${install_root}/rules" "${install_root}/tls" "${install_root}/data" "${install_root}/logs"
fi
...
if [[ ! -f "${config_file}" || "${force_config}" -eq 1 ]]; then
  ...
  if [[ "${source_checkout}" -eq 1 ]]; then
    config_source="${script_dir}/configs/config.yaml"
  else
    config_source="${script_dir}/config/config.yaml"
  fi
  if [[ "${source_checkout}" -eq 0 && "${force_config}" -eq 0 && -f "${legacy_config_file}" && "${config_file}" != "${legacy_config_file}" ]]; then
...
if [[ "${source_checkout}" -eq 0 && -d "${legacy_tls_dir}" && "${install_root}/tls" != "${legacy_tls_dir}" ]]; then
...
if [[ "${source_checkout}" -eq 0 ]]; then
  ...
  sed "s|@INSTALL_ROOT@|${install_root}|g" "${script_dir}/deploy/linux/jingshield.service" > "${service_file}"
  chmod 0644 "${service_file}"
fi
if [[ "${source_checkout}" -eq 0 && -d "${script_dir}/rules" ]]; then
...
if ! "${install_root}/jingshield" migrate -c "${config_file}"; then
  if [[ -t 0 ]]; then
    ...
    echo
    [[ -n "${db_password}" && "${db_password}" =~ ^[A-Za-z0-9._~!@%+=:-]+$ ]] || {
      echo "MySQL 密码为空或包含不支持的字符" >&2
      exit 1
    }
    export JINGSHIELD_DB_PASS="${db_password}"
...
if [[ "${source_checkout}" -eq 1 ]]; then
  if [[ "${no_start}" -eq 0 ]]; then
    source_cert_file="${JINGSHIELD_TLS_CERT_FILE:-${install_root}/tls/jingshield.crt}"
    source_key_file="${JINGSHIELD_TLS_KEY_FILE:-${install_root}/tls/jingshield.key}"
    if grep -Eq '^[[:space:]]*tls_listen:[[:space:]]*"[^"[:space:]]+' "${config_file}" && \
        [[ ! -s "${source_cert_file}" || ! -s "${source_key_file}" ]]; then
      install -d -m 0750 "$(dirname "${source_cert_file}")"
      source_node_ip="$(hostname -I 2>/dev/null | awk '{print $1}')"
      source_hosts="localhost,127.0.0.1"
      [[ -n "${source_node_ip}" ]] && source_hosts="${source_hosts},${source_node_ip}"
      "${install_root}/jingshield" cert --cert "${source_cert_file}" --key "${source_key_file}" --hosts "${source_hosts}" --days 10950
      chmod 0644 "${source_cert_file}"
      chmod 0600 "${source_key_file}"
    fi
    echo "JingShield repository mode: starting in foreground; no /opt or /etc files were changed"
    exec "${install_root}/jingshield" -c "${config_file}"
  else
    echo "JingShield repository mode: initialized without start config=${config_file}"
  fi
