...
import { CloudDownload, Download, FileJson, Pencil, Plus, RefreshCw, Sparkles, Trash2, Upload, X } from '@lucide/vue'
...
const maxRulePackBytes = 2 * 1024 * 1024
...
function openImport() { importError.value = ''; importFileName.value = ''; showImport.value = true }
...
async function importRules() {
  ...
  try {
    const { pack, formatted } = parseRulePack(importJSON.value)
    importJSON.value = formatted
    const result = await apiRequest<{ count: number }>('/policies/import', { method: 'POST', ...jsonBody(pack) })
    showImport.value = false; flash(`已原子导入 ${result.count} 条策略`); await load()
  } catch (reason) { importError.value = message(reason, '规则包不是合法 JSON 或导入失败') }
  finally { importing.value = false }
}
function parseRulePack(text: string) {
  const normalized = text.replace(/^\uFEFF/, '')
  const pack: unknown = JSON.parse(normalized)
  if (typeof pack !== 'object' || pack === null || Array.isArray(pack)) throw new Error('规则包顶层必须是 JSON 对象')
  const record = pack as Record<string, unknown>
  const unknown = Object.keys(record).filter(key => !['schema', 'version', 'rules'].includes(key))
  if (unknown.length) throw new Error(`规则包包含未知字段：${unknown[0]}`)
  if (record.schema !== 'jingshield.rules/v1') throw new Error('规则包 schema 必须是 jingshield.rules/v1')
  if (typeof record.version !== 'string' || !/^[A-Za-z0-9._-]{1,50}$/.test(record.version)) throw new Error('规则包版本只能包含字母、数字、点、下划线和连字符')
  if (!Array.isArray(record.rules) || record.rules.length < 1 || record.rules.length > 5000) throw new Error('规则数量必须为 1–5000 条')
  return { pack, formatted: JSON.stringify(pack, null, 2) }
}
async function selectImportFile(event: Event) {
  ...
  if (!file) return
  importError.value = ''
  if (!/\.json$/i.test(file.name)) { importError.value = '只允许选择 .json 文件'; return }
  if (file.type && file.type !== 'application/json' && file.type !== 'text/json') { importError.value = '文件 MIME 类型必须是 application/json'; return }
  if (file.size < 1 || file.size > maxRulePackBytes) { importError.value = 'JSON 规则包大小必须在 1 字节到 2MB 之间'; return }
  try {
    const { formatted } = parseRulePack(await file.text())
    importJSON.value = formatted
    importFileName.value = file.name.slice(0, 255)
  } catch (reason) { importError.value = message(reason, '无法读取 JSON 规则包') }
}
...
<template>
  <section class="page-content">
    ...
    <div v-if="showImport" class="modal-layer"><div class="modal-card wide-modal"><div class="modal-heading"><div><p class="eyebrow">OFFLINE IMPORT</p><h2>导入规则包</h2></div><button @click="showImport = false"><X :size="20" /></button></div><p class="field-help">导入采用事务替换，仅覆盖上一次离线导入的规则。文件仅在浏览器读取，不上传原始文件、不使用文件名落盘，也不会实例化 JSON 中的类型。</p><input ref="importFileInput" hidden type="file" accept=".json,application/json" @change="selectImportFile" /><button class="rule-upload-picker" type="button" @click="importFileInput?.click()"><FileJson :size="23" /><span><strong>选择 JSON 规则包</strong><small>仅限 .json / application/json，最大 2MB</small></span><b>{{ importFileName || '选择文件' }}</b></button><p v-if="importError" class="inline-alert import-alert">{{ importError }}</p><textarea v-model="importJSON" class="policy-textarea import-area" spellcheck="false"></textarea><div class="modal-actions"><button class="secondary-button" :disabled="importing" @click="downloadTemplate"><Download :size="15" /> 下载模板</button><button class="primary-button" :disabled="importing" @click="importRules"><Upload :size="15" /> {{ importing ? '校验中…' : '校验并导入' }}</button></div></div></div>
