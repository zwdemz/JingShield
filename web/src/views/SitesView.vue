...
const form = reactive({ name: '', host: '', upstream: 'http://127.0.0.1:9000', enabled: true, pass_host: true, tls_skip_verify: false })
...
function openCreate() {
  ...
  Object.assign(form, { name: '', host: '', upstream: 'http://127.0.0.1:9000', enabled: true, pass_host: true, tls_skip_verify: false })
  showForm.value = true
}
  ...
  <section class="page-content">
    ...
    <div v-else class="site-grid">
      <article v-for="site in sites" :key="site.id" class="panel site-card" :class="{ disabled: !site.enabled }">
        ...
        <div class="site-card-foot"><span>更新于 {{ formatTime(site.updated_at) }}</span><div><button title="编辑站点" @click="openEdit(site)"><Pencil :size="16" /></button><button class="danger-icon" title="删除站点" @click="remove(site)"><Trash2 :size="16" /></button></div></div>
      </article>
    </div>
