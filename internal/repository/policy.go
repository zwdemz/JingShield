...
const policyColumns = "id, name, category, target, pattern, action, enabled, priority, source, version, COALESCE(description, ''), COALESCE(cve_id, ''), COALESCE(component, ''), COALESCE(affected_versions, ''), confidence, COALESCE(exploit_stage, ''), created_at, updated_at"

func scanPolicy(scanner interface{ Scan(...any) error }) (*model.PolicyRule, error) {
	rule := &model.PolicyRule{}
	err := scanner.Scan(&rule.ID, &rule.Name, &rule.Category, &rule.Target, &rule.Pattern, &rule.Action, &rule.Enabled, &rule.Priority, &rule.Source, &rule.Version, &rule.Description, &rule.CVEID, &rule.Component, &rule.AffectedVersions, &rule.Confidence, &rule.ExploitStage, &rule.CreatedAt, &rule.UpdatedAt)
	return rule, err
}
...
func (r *PolicyRepo) Create(ctx context.Context, rule *model.PolicyRule) error {
	res, err := r.db.ExecContext(ctx, `INSERT INTO jyj_policy_rules
		(name, category, target, pattern, action, enabled, priority, source, version, description, cve_id, component, affected_versions, confidence, exploit_stage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		rule.Name, rule.Category, rule.Target, rule.Pattern, rule.Action, rule.Enabled, rule.Priority, rule.Source, rule.Version, rule.Description, rule.CVEID, rule.Component, rule.AffectedVersions, rule.Confidence, rule.ExploitStage)
	if err != nil {
		return err
	}
...
func (r *PolicyRepo) Update(ctx context.Context, rule *model.PolicyRule) error {
	res, err := r.db.ExecContext(ctx, `UPDATE jyj_policy_rules SET name=?, category=?, target=?, pattern=?, action=?, enabled=?, priority=?, version=?, description=?, cve_id=?, component=?, affected_versions=?, confidence=?, exploit_stage=? WHERE id=?`,
		rule.Name, rule.Category, rule.Target, rule.Pattern, rule.Action, rule.Enabled, rule.Priority, rule.Version, rule.Description, rule.CVEID, rule.Component, rule.AffectedVersions, rule.Confidence, rule.ExploitStage, rule.ID)
	if err != nil {
		return err
	}
...
func (r *PolicyRepo) ReplaceSource(ctx context.Context, source string, rules []*model.PolicyRule) error {
	...
	for _, rule := range rules {
		if _, err := stmt.ExecContext(ctx, rule.Name, rule.Category, rule.Target, rule.Pattern, rule.Action, rule.Enabled, rule.Priority, source, rule.Version, rule.Description, rule.CVEID, rule.Component, rule.AffectedVersions, rule.Confidence, rule.ExploitStage); err != nil {
			return err
		}
	}
	return tx.Commit()
