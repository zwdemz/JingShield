...
var schemaMigrations = []migration{
  ...
  affected_versions VARCHAR(255) NULL,
  confidence TINYINT NOT NULL DEFAULT 0,
  exploit_stage VARCHAR(30) NULL,
  ...
  enabled TINYINT(1) NOT NULL DEFAULT 1,
  pass_host TINYINT(1) NOT NULL DEFAULT 0,
...
func Migrate(ctx context.Context, db *sql.DB) error {
	...
	for _, column := range []struct{ name, ddl string }{
		{"cve_id", "ALTER TABLE jyj_policy_rules ADD COLUMN cve_id VARCHAR(64) NULL AFTER description"},
		{"component", "ALTER TABLE jyj_policy_rules ADD COLUMN component VARCHAR(100) NULL AFTER cve_id"},
		{"affected_versions", "ALTER TABLE jyj_policy_rules ADD COLUMN affected_versions VARCHAR(255) NULL AFTER component"},
		{"confidence", "ALTER TABLE jyj_policy_rules ADD COLUMN confidence TINYINT NOT NULL DEFAULT 0 AFTER affected_versions"},
		{"exploit_stage", "ALTER TABLE jyj_policy_rules ADD COLUMN exploit_stage VARCHAR(30) NULL AFTER confidence"},
	} {
		if _, err := ensureColumn(ctx, db, "jyj_policy_rules", column.name, column.ddl); err != nil {
			return err
		}
	}
